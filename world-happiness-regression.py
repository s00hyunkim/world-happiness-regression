...
# 2-2. check if there are any null values
data.isnull().sum()
# 2-3. determine what % of each feature is null to figure out which null values to drop
print('[% OF NULL VALUES OF EACH FEATURE')
for col in data.columns:
    print(col,  ': %.2f%%' %((data[col].isnull().sum() / data.shape[0]) * 100))
print('')
# 2-4. drop null values of the features that have the least %
...
data.info()
# 2-6. for other null values, use euclidean distance & uniform weights
...
# 2-7. check how many non-null values each feature has
#      (all feature should have the same # of non-null values at this step)
data.info()
# 2-8. check correlation between each feature and target
