...
st.title('Welcome!')
st.write("We are Team 9 (Vatsal Bora, Shivani Chidella, Soohyun Kim, Louis Liu).")
st.write("The primary objective of our project is to help people with an interest in subjective well-being (such as politicians and bureaucrats) understand how different factors impact a Happiness Score so that they can try improving it.")
...
gdp = c3.number_input('Insert the value for GDP. 95% conf. int: {}'.format(tuple([round(x,2) for x in gdp_confint])))
ss = c3.number_input('Insert the value for Social Support. 95% conf. int: {}'.format(tuple([round(x,2) for x in ss_confint])))
hle = c3.number_input('Insert the value for Healthy Life Expectancy. 95% conf. int: {}'.format(tuple([round(x,2) for x in hle_confint])))
fre = c4.number_input('Insert the value for Freedom. 95% conf. int: {}'.format(tuple([round(x,2) for x in fre_confint])))
gen = c4.number_input('Insert the value for Generosity. 95% conf. int: {}'.format(tuple([round(x,2) for x in gen_confint])))
cor = c4.number_input('Insert the value for Corruption. 95% conf. int: {}'.format(tuple([round(x,2) for x in cor_confint])))
...
st.markdown("<h5 style='text-align: left;'>Important notes on units and meaning of features (note that GWP refers to the Gallup World Poll (GWP)):</h5>", unsafe_allow_html=True)
